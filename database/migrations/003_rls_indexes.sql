...
CREATE POLICY tenant_isolation_graphs ON attack_graphs
  USING (organization_id = current_setting('app.current_org', true)::uuid);
...
CREATE INDEX IF NOT EXISTS idx_alerts_org_open ON alerts(organization_id, created_at DESC) WHERE status IN ('new', 'triaging', 'escalated');
CREATE INDEX IF NOT EXISTS idx_incidents_org_open ON incidents(organization_id, opened_at DESC) WHERE status IN ('open', 'investigating', 'contained');
